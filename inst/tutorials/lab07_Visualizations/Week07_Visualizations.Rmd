...
So far, we covered several techniques for understanding networks and we will cover more in the following weeks. Still, sometimes the best analysis is not very much appreciated by your audience if it is difficult to understand it.

Imagine that your boss asked you an overview of network data of interest and you show up with a pdf file filled with tables and diagrams. It's not gonna fly if your boss has 5 minutes for you, right?

With this tutorial we will learn how to represent networks in pretty and informative ways, so that your analyses will have a greater impact on the intended audience. If you do the job well, the 5 minutes that your boss has for you will be more than enough to support your point!
...
larger public. Communicability is an essential skill for academia but also for
...
Hence, either you import your data from a data.frame/matrix/edgelist etc into `igraph`, or make use of an existing `igraph` object, or use the package `intergraph` to change the class of your network to `igraph`. 
...
So, when I said your story, I mean properly your story! The data that we are going to visualize is YOUR data. 
It's the survey that we did together in week 4. Well, something close to your story to be honest, since I anonymized it and change it a bit, so no one can reconize themselves! 
...
Simply plot it reducing the vertexes and the arrows size, removing the vertex label and curving the edges.
...
Now we actually see the groups and the relationships in a clear way.

Still, we can do better! Let's do it, together with some aesthetic improvements!
...
It surely looks fancier, but it is still not telling us a lot about what happens in your group!
...
With this visualization we can already see whether the year and the gender matters in forming friendship!
...
Pay attention: since the simple numeric variable expressing Data Science likelihood would plot nodes of a very small size, I added `*2` to multiply the coefficient by 2 and increase the size. It's an helpful trick. 
...
Another common and useful way to show a node attribute on a plot is to show different shades of the same color to show how the "intensity" or an attribute increases. 
Obviously this makes sense with ordinal variables. Let's try with `Hang.Out`. 

We know it ranges from 1 never hang-out with friends, to 5 hang-out every day.

We prepare a new variable with the color before inserting it into the plot so we can manually assign the colors since random ones won't do the trick. We then move on plotting and finally we add a legend. 
...
We also don't care about showing a node attribute with the color, since we want to use colors to show the edge attribute. Too many colors are confusing. 
...
If we only want to show that these friendship have different frequency, we can simply set the edge attribute as the edge color and random colors will be plotted characterizing different relationships. 
...
It worked, but this is not very informative. You can make a color scale as we did for the node, or you can plot the attribute as the tickness of the edge. 

Let's try using the parameter `edge.width` and showing the remaing variable closeness!
...
In this tutorial we sow an overview of the most useful techniques to plot networks in `igraph`. Obviously there are many more tricks and we will se some other during the lab. 

Still, the most important thing to remember is that what really matters is that you know which story you want to tell with your visualization and you make sure it shows in the plot. 
